<div class="container">	 
	<table border="0">
		<tr class="bidderUIHeaderRow">
			<td>
				month
				<a href="/reports?entity={$entity}&timeUnit=allTime&campaignId={$campaignId}&placementId={$placementId}&exchangeId={$exchangeId}"><img
		...
		{foreach from=$rows key=key item=row}
			<tr class="bidderUIRow">
				<td align="right">
					<a href="/reports?entity={$entity}&timeUnit=month&time={$row.month}&campaignId={$campaignId}&placementId={$placementId}&exchangeId={$exchangeId}">{$row.month}</a>
				</td>
