<div class="container">	 
	<table border="0">
		<tr class="bidderUIHeaderRow">
			<td>
				date
				<a href="/reports?entity={$entity}&timeUnit=year&time={$drillUpTime}&campaignId={$campaignId}&placementId={$placementId}&exchangeId={$exchangeId}"><img
		...
		{foreach from=$rows key=key item=row}
			<tr class="bidderUIRow">
				<td align="right">
					<a href="/reports?entity={$entity}&timeUnit=day&time={$row.date}&campaignId={$campaignId}&placementId={$placementId}&exchangeId={$exchangeId}">{$row.date}</a>
				</td>
