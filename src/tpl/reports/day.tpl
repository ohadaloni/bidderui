<div class="container">	 
	<table border="0">
		<tr class="bidderUIHeaderRow">
			<td>
				hour
				<a href="/reports?entity={$entity}&timeUnit=month&time={$drillUpTime}&campaignId={$campaignId}&placementId={$placementId}&exchangeId={$exchangeId}"><img
		...
		{foreach from=$rows key=key item=row}
			<tr class="bidderUIRow">
				<td align="right">
					<a href="/reports?entity={$entity}&timeUnit=hour&time={$row.date}-{$row.hour}&campaignId={$campaignId}&placementId={$placementId}&exchangeId={$exchangeId}">{$row.hour}</a>
				</td>
